...
#define matrix of predictor variables
xp <- dataint[, c("Ozone", "PM2.5", "DieselPM", "LinguisticIsolation",  "Poverty", "Unemployment", "LowBirthWeight")] |> as.matrix()
elasticnetp = glmnet(xp,yp,alpha=0.5,family="gaussian")
vip(elasticnetp, num_features=10, geom = "point")
#now we look at the Variable importance (vip) for factors excluding Ozone
xp_noOzone <- data_cal1[, c("PM2.5", "DieselPM", "LinguisticIsolation",  "Poverty", "Unemployment", "LowBirthWeight")] |> as.matrix()
...
```{r}
mod.poisson.sim <- glm(Asthma ~ Ozone + PM2.5 + DieselPM + LinguisticIsolation + Poverty + Unemployment+ LowBirthWeight, data=dataint, family=quasipoisson)
...
```{r}
#
modp.pm_ozone <- glm(Asthma ~ Ozone + PM2.5 + DieselPM + LinguisticIsolation + Poverty + Unemployment+ LowBirthWeight + Ozone * PM2.5, data=dataint, family=quasipoisson)
modp.ozone_diesel <-glm(Asthma ~ Ozone + PM2.5 + DieselPM + LinguisticIsolation + Poverty + Unemployment+ LowBirthWeight + Ozone * DieselPM, data=dataint, family=quasipoisson)
modp.ozone_linguisticIsolation <- glm(Asthma ~ Ozone + PM2.5 + DieselPM + LinguisticIsolation + Poverty + Unemployment+ LowBirthWeight + Ozone * LinguisticIsolation, data=dataint, family=quasipoisson)
modp.ozone_poverty <- glm(Asthma ~ Ozone + PM2.5 + DieselPM + LinguisticIsolation + Poverty + Unemployment+ LowBirthWeight + Ozone * Poverty, data=dataint, family=quasipoisson)
modp.ozone_unemployment <- glm(Asthma ~ Ozone + PM2.5 + DieselPM + LinguisticIsolation + Poverty + Unemployment+ LowBirthWeight + Ozone * Unemployment, data=dataint, family=quasipoisson)
modp.ozone_lowbirthweight <- glm(Asthma ~ Ozone + PM2.5 + DieselPM + LinguisticIsolation + Poverty + Unemployment+ LowBirthWeight + Ozone * LowBirthWeight, data=dataint, family=quasipoisson)
...
As can see from the table above, the interaction term of Ozone and DieselPM is not significant, but all other interaction terms have p-values lower than the significance level. 
...
Since PM 2.5 and DieselPM are both air pollution factors, we also examine the confounding effect and interactions between these two terms. 

first We want to approximate the effects of PM 2.5 and Asthma:
```{r}
...
The model with Pm 2.5 alone has very great overdispersion, at the value of 15.5. 

From the summary statistics: we can see that every 1 unit increase in PM 2.5 concentration is estimated to be associated with, on average, 2% increase in the incidence rate of asthma among individuals that live in the counties with the current level of PM2.5 concentration.
...
```{r}
mod.p_pmdinter <- glm(Asthma ~ Ozone + PM2.5 + DieselPM + LinguisticIsolation + Poverty + Unemployment+ LowBirthWeight +PM2.5 * DieselPM, data=dataint, family=poisson())
summary(mod.p_pmdinter)
deviance(mod.p_pmdinter)/mod.p_pmdinter$df.residual
...
Next, since PM 2.5, Diesel PM and Ozone are the air pollution factors, which are most likely to be directly associated with Asthma rate, we examine the effects of adding in the quadratic terms of these variables. 

Incoporating the quadratic terms of PM2.5, Diesel PM, Ozone into the model:
...
```{r}
mod.poisson.qua.pm2.5diesel <- glm(Asthma ~ Ozone + PM2.5 + DieselPM + LinguisticIsolation + 
    Poverty + Unemployment+ LowBirthWeight  + I(Ozone^2) + I(PM2.5^2) + I(DieselPM ^2) + PM2.5*DieselPM, data=dataint, family=quasipoisson)
summary(mod.poisson.qua.pm2.5diesel)

```
The interaction term of PM2.5 and DieselPM does not have a p-value as significant as the other terms, but the chisq test has a p-value under the significance level, which indicates that it's sufficient to reject the null hypothesis and conclude that the including the interaction term is better in predicting Asthma rates. 

```{r}
anova(mod.poisson.qua,mod.poisson.qua.pm2.5diesel, test ='Chisq')
```
...
mod.poisson.qua.pm2.5ozone <- glm(Asthma ~ Ozone + PM2.5 + DieselPM + LinguisticIsolation + 
    Poverty + Unemployment+ LowBirthWeight+ I(Ozone^2) + I(PM2.5^2) + I(DieselPM ^2) + PM2.5*Ozone, data=dataint, family=quasipoisson)
...
summary(mod.poisson.qua.pm2.5diesel)$dispersion
summary(mod.poisson.qua.pm2.5ozone)$dispersion
...
```{r}
modnames <- c('mod.poisson.sim','modp.pm_ozone','modp.ozone_diesel','modp.ozone_linguisticIsolation','modp.ozone_poverty','modp.ozone_lowbirthweight','mod.poisson.qua','od.poisson.qua.pm2.5diesel','mod.poisson.qua.pm2.5ozone')
poisson_model_dispersions <- c(summary(mod.poisson.sim)$dispersion,
summary(modp.pm_ozone)$dispersion,
summary(modp.ozone_diesel)$dispersion,
summary(modp.ozone_linguisticIsolation)$dispersion,
summary(modp.ozone_poverty)$dispersion,
summary(modp.ozone_lowbirthweight)$dispersion,
summary(mod.poisson.qua)$dispersion,
summary(mod.poisson.qua.pm2.5diesel)$dispersion,
summary(mod.poisson.qua.pm2.5ozone)$dispersion)

cbind(modnames,poisson_model_dispersions)
```
...
nbin1 <- MASS::glm.nb(Asthma ~ Ozone + PM2.5 + DieselPM + LinguisticIsolation + 
    Poverty + Unemployment+ LowBirthWeight +  I(Ozone^2) + I(PM2.5^2) + I(DieselPM ^2) + PM2.5 * Ozone, data = dataint)
summary(nbin1)
```
```{r}
