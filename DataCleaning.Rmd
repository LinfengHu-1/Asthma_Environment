...
soecon$SNAP <- as.numeric(gsub(",", "", soecon$SNAP))/10000
soecon$individual_homeless <- as.numeric(soecon$individual_homeless) /10000
soecon$fam_homeless <- as.numeric(soecon$fam_homeless)/10000
soecon$total_homeless <- as.numeric(soecon$total_homeless)/10000


```

```{r}
#write.csv(soecon,'soecon.csv')
```
...
```{r}
one<- merge(homeless, unrate, by = c("Location" = "State", "year"= "year"))
two <- merge(snap, one, by= c('Location', 'year'))
soecon <- merge(two, poverty, by = c('Location', 'year'))
names(soecon)[names(soecon) == 'Individuals'] <- 'individual_homeless'
names(soecon)[names(soecon) == 'Total People Experiencing Homelessness'] <- 'total_homeless'
...
```
```{r}
dat_exp <- read.csv("dat_exp.csv")
soecon <- read.csv("soecon.csv")
...
par(mfrow = c(3,2))
boxplot(dat_full$DataValue ~ dat_full$o3_mean)
boxplot(dat_full$DataValue ~ dat_full$pm2.5_mean)
boxplot(dat_full$DataValue ~ dat_full$so2_mean)
boxplot(dat_full$DataValue ~ dat_full$co_mean)
boxplot(dat_full$DataValue ~ dat_full$no2_mean)
